...
fname <- paste0(figdir, "appendixFigS5.pdf")
setwd(basedir)
...
########## Left column
dt <- read.table(paste0(datadir, "Evodyn-RS010-T1000-GV02.out"))
...
legend(1000, 1.025 * par("usr")[4], c("Nursery habitat", "Harvested area", "Marine reserve"), 
       col = c("black", "#0072B2", "#D55E00"), lwd = baselwd, cex = cexleg, horiz = T, bty = "n", adj = c(0, 0.5))
polygon(c(1050, 7000, 7000, 1050), c(rep(1.0, 2), rep(0.85, 2)) * par("usr")[4], col = NA, xpd = T)
...
dt <- read.table(paste0(datadir, "Evodyn-RS030-T1000-GV02.out"))
