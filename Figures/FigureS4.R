...
fname <- paste0(figdir, "appendixFigS4.pdf")
setwd(basedir)
...
########## Left column
dt <- read.table(paste0(datadir, "Evodyn-RS010-T1000-ETA1_05.out"))

par(mar = c(panelsep, lmarl, 5.8, rmar))
plot(NULL, NULL, xlim = xliml, ylim = ylimb, 
     xlab = "", ylab = "", xaxs = "i", yaxs = "i", 
     xaxt = "n", yaxt = "n", bty = "l")
polygon(c(0, 1000, 1000, 0), par("usr")[c(3, 3, 4, 4)], col = "lightgrey")
...
Arrows(120.0, 1.08 * par("usr")[4],  880.0, 1.08 * par("usr")[4], arr.type="triangle", lwd = 2, arr.width = 0.2, code = 3, xpd = T)
Arrows(1120.0, 1.08 * par("usr")[4], 6880.0, 1.08 * par("usr")[4], arr.type="triangle", lwd = 2, arr.width = 0.2, code = 3, xpd = T)
text(500, 1.30 * par("usr")[4], "No", cex = 2.0, xpd = T)
text(500, 1.18 * par("usr")[4], "reserve", cex = 2.0, xpd = T)
text(4000, 1.25 * par("usr")[4], "10% protected", cex = cexttl, xpd = T)

legend(1000, 1.015 * par("usr")[4], c("Nursery habitat", "Harvested area", "Marine reserve"), 
       col = c("black", "#0072B2", "#D55E00"), lwd = baselwd, cex = cexleg, horiz = T, bty = "n", adj = c(0, 0.5))
polygon(c(1050, 7000, 7000, 1050), c(rep(0.995, 2), rep(0.845, 2)) * par("usr")[4], col = NA, xpd = T)
...
dt <- read.table(paste0(datadir, "Evodyn-RS030-T1000-ETA1_05.out"))
...
text(500, 1.30 * par("usr")[4], "No", cex = 2.0, xpd = T)
text(500, 1.18 * par("usr")[4], "reserve", cex = 2.0, xpd = T)
text(4000, 1.25 * par("usr")[4], "30% protected", cex = cexttl, xpd = T)
