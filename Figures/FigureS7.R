...
fname <- paste0(figdir, "appendixFigS7.pdf")
setwd(basedir)
...
zlabs   <- (0:5) * 0.004
zconts  <- seq(0, 0.016, length.out = 10)
zclrs   <- seq(0, 0.016, length.out = 200)
selectedcol <- EBTh2adubiocol
...
for (ii in (1:length(bmortvals))) {
  fn <- paste0(sub('\\.', "", paste0(datadir, "Heatmaps-Eco/SR00-ETSbif-Bmort-", sprintf("%.2f", bmortvals[ii]))), ".gavg.out")
  if (file.size(fn) > 0) {
    dt <- read.table(fn)
    dt <- dt[(dt[,EBTbifparcol] <= ylim[2]), selectedcol]
    SR00_ETSbif_Bmort_var[ii, (1:length(dt))] <- fmortvals[(1:length(dt))] * dt
  }
  
  fn <- paste0(sub('\\.', "", paste0(datadir, "Heatmaps-Eco/SR01-ETSbif-Bmort-", sprintf("%.2f", bmortvals[ii]))), ".gavg.out")
  if (file.size(fn) > 0) {
    dt <- read.table(fn)
    dt <- dt[(dt[,EBTbifparcol] <= ylim[2]), selectedcol]
    SR01_ETSbif_Bmort_var[ii, (1:length(dt))] <- fmortvals[(1:length(dt))] * dt
  }
  
  fn <- paste0(sub('\\.', "", paste0(datadir, "Heatmaps-Eco/SR03-ETSbif-Bmort-", sprintf("%.2f", bmortvals[ii]))), ".gavg.out")
  if (file.size(fn) > 0) {
    dt <- read.table(fn)
    dt <- dt[(dt[,EBTbifparcol] <= ylim[2]), selectedcol]
    SR03_ETSbif_Bmort_var[ii, (1:length(dt))] <- fmortvals[(1:length(dt))] * dt
  }
}

maxval = max(cbind(SR00_ETSbif_Bmort_var, SR01_ETSbif_Bmort_var, SR03_ETSbif_Bmort_var), na.rm = T)

SR00_ETSbif_Bmort_var <- pmin(SR00_ETSbif_Bmort_var, max(zclrs), na.rm = T)
SR01_ETSbif_Bmort_var <- pmin(SR01_ETSbif_Bmort_var, max(zclrs), na.rm = T)
SR03_ETSbif_Bmort_var <- pmin(SR03_ETSbif_Bmort_var, max(zclrs), na.rm = T)
...
mtext("Average yield of adult biomass per unit time", 3, cex = cexttl, line = 5.0)
...
par(mar=c(bmar, 0, tmart, rmarR), mgp = c(3, 0.3, 0))
plot.new()
...
for (ii in (1:length(bmortvals))) {
  fn <- paste0(sub('\\.', "", paste0(datadir, "Heatmaps-Evo/SR00-ETSbif-Bmort-", sprintf("%.1f", bmortvals[ii]))), ".gavg.out")
  if (file.size(fn) > 0) {
    dt <- read.table(fn)
    dt <- dt[(dt[,EBTbifparcol] <= ylim[2]), selectedcol]
    SR00_ETSbif_Bmort_var[ii, (1:length(dt))] <- fmortvals[(1:length(dt))] * dt
  ...
  fn <- paste0(sub('\\.', "", paste0(datadir, "Heatmaps-Evo/SR01-ETSbif-Bmort-", sprintf("%.1f", bmortvals[ii]))), ".gavg.out")
  if (file.size(fn) > 0) {
    dt <- read.table(fn)
    dt <- dt[(dt[,EBTbifparcol] <= ylim[2]), selectedcol]
    SR01_ETSbif_Bmort_var[ii, (1:length(dt))] <- fmortvals[(1:length(dt))] * dt
  ...
  fn <- paste0(sub('\\.', "", paste0(datadir, "Heatmaps-Evo/SR03-ETSbif-Bmort-", sprintf("%.1f", bmortvals[ii]))), ".gavg.out")
  if (file.size(fn) > 0) {
    dt <- read.table(fn)
    dt <- dt[(dt[,EBTbifparcol] <= ylim[2]), selectedcol]
    SR03_ETSbif_Bmort_var[ii, (1:length(dt))] <- fmortvals[(1:length(dt))] * dt
...
maxval = max(c(maxval, max(cbind(SR00_ETSbif_Bmort_var, SR01_ETSbif_Bmort_var, SR03_ETSbif_Bmort_var), na.rm = T)), na.rm = T)
...
par(mar=c(bmar, 0, tmar, rmarR), mgp = c(3, 0.3, 0))
plot.new()
plot.window(xlim = c(0, 1), ylim = zlim, xaxs = "i", yaxs = "i")
rect(0, zclrs[-length(zclrs)], 1, zclrs[-1L], density = -1, col = clrs, lwd = 0, border = "transparent")
if (drawconts) rect(0, zconts[-length(zconts)], 1, zconts[-1L], col = NA, lwd = 0.1)
